...
import {
  PublicTourKeyPoints,
  TourKeyPoints,
} from 'src/app/feature-modules/tour-authoring/model/tour-keypoints.model';
...
import {
  PositionSimulator,
  Session,
} from 'src/app/feature-modules/tour-execution/model/position-simulator.model';
...
import {
  ...
  ChallengeStatus,
  ChallengeType,
} from 'src/app/feature-modules/administration/model/challenge.model';
...
export class MapViewComponent implements OnInit {
  ...
  loadChallenges() {
    this.service.getChallenges().subscribe({
      next: (result: PagedResults<Challenge>) => {
        this.challenges = result.results.filter(
          (c) => c.keyPointId == null && c.status == ChallengeStatus.Active
        );
        this.service.getChallengeExecutionForTourist(this.user.id).subscribe({
          next: (result: PagedResults<ChallengeExecution>) => {
            ...
            this.activeChallenge = result.results.find(
              (challengeExecution) => challengeExecution.isCompleted === false
            );
            ...
            this.challenges = this.challenges.filter(
              (challenge) =>
                !result.results.some(
                  (challengeExecution) =>
                    challengeExecution.challengeId === challenge.id &&
                    challengeExecution.isCompleted == true
                )
            );
  ...
  continueActiveChallenge(challengeExecutions: ChallengeExecution[]) {
    this.activeChallenge = challengeExecutions.find(
      (challengeExecution) => challengeExecution.isCompleted === false
    );
    if (!this.activeChallenge) return;
    this.nearbyChallenge = this.challenges.find(
      (challenge) => challenge.id === this.activeChallenge?.challengeId
    );
    ...
    if (this.nearbyChallenge?.type === ChallengeType.Misc) {
      ...
      this.openChallengeDialog(
        this.nearbyChallenge!.name!,
        this.nearbyChallenge!.description!,
        2
      );
      ...
      if (this.activeChallenge?.isCompleted) {
        this.toastr.warning(
          `You have completed social challenge: ${this.nearbyChallenge!.name}`,
          'Warning'
        );
        this.service
          .addXPSocial(
            this.nearbyChallenge!.id!,
            this.nearbyChallenge!.experiencePoints!
          )
          .subscribe();
      ...
      this.toastr.warning(
        `You have activated social challenge: ${
          this.nearbyChallenge!.name
        } to complete the challenge ${
          this.nearbyChallenge!.requiredAttendance
        } people need to activate this challenge`,
        'Warning'
      );
      this.showStartChallenge = false;
    }
  ...
  private enableTourRating() {
    this.tours.forEach((tour) => {
      tour.canBeRated = this.sessions.some(
        (session) =>
          session.tourId == tour.id && session.distanceCrossedPercent! > 30
      );
    });
  }
          ...
          complete: () => {
            ...
            this.route.queryParams.subscribe((params) => {
              if (params['tourId'] != undefined) {
                this.showTourOnMap(
                  this.tours.find((t) => t.id == params['tourId'])!
                );
                this.selectedTour.selectedTransport =
                  this.selectedTour.durations[0].transportation;
              }
            });
          },
        });
  ...
  showTourOnMap(tour: Tour) {
    ...
    switch (this.user.role) {
      ...
      case 'tourist':
        tour.keyPoints.forEach((keypoint) => {
          this.mapComponent.addTouristKeyPointMarker(
            keypoint,
            this.activeSession === null ? false : true,
            true
          );
          this.mapComponent.setRoute(vehicleType);
        });
  ...
  setMyLocation(): void {
    this.service.getPositionSimulatorByTouristId(this.user.id).subscribe({
      next: (result: PositionSimulator) => {
        ...
        this.mapComponent.addTouristMarker(
          this.positionSimulator.latitude,
          this.positionSimulator.longitude
        );
  ...
  addLocation(): void {
    ...
    this.service.createPositionSimulator(positionSimulator).subscribe({
      ...
      complete: () => {
        this.service
          .update(this.person, this.user.id, this.user.role)
          .subscribe({
            next: (result: Person) => {
              this.person = result;
            },
  ...
  updateMyLocation(): void {
    this.positionSimulator.latitude = this.mapComponent
      .getTouristMarker()
      .getLatLng().lat;
    this.positionSimulator.longitude = this.mapComponent
      .getTouristMarker()
      .getLatLng().lng;
  ...
  updateActiveChallenge() {
    ...
    if (this.nearbyChallenge?.type! === ChallengeType.Social) {
      if (
        this.mapComponent.getDistance(
          this.nearbyChallenge!.latitude,
          this.nearbyChallenge!.longitude,
          this.positionSimulator.latitude,
          this.positionSimulator.longitude
        ) > this.nearbyChallenge!.range!
      ) {
        this.service
          .deleteChallengeExecution(this.activeChallenge.id!)
          .subscribe({
            next: () => {
              this.toastr.warning(
                `You have left the ${this.nearbyChallenge?.name} social challenge`,
                'Warning'
              );
              this.activeChallenge = undefined;
              this.nearbyChallenge = undefined;
        ...
        .subscribe({
          next: (result: ChallengeExecution) => {
            ...
            if (this.nearbyChallenge?.type === ChallengeType.Location) {
              if (
                this.mapComponent.getDistance(
                  this.nearbyChallenge!.latitudeImage!,
                  this.nearbyChallenge!.longitudeImage!,
                  this.activeChallenge!.latitude,
                  this.activeChallenge!.longitude
                ) <= 30
              ) {
                ...
                if (this.showLocationChallengeCompleted == false) {
                  this.openChallengeDialog(
                    this.nearbyChallenge.name!,
                    'You have found the location where the picture was taken. Enjoy the view for a few seconds to complete the location challenge!',
                    -1
                  );
                }
  ...
  canChallengeStart() {
    if (this.activeChallenge) return;
    this.nearbyChallenge = this.displayedChallenges.find(
      (challenge) =>
        this.mapComponent.getDistance(
          this.positionSimulator.latitude,
          this.positionSimulator.longitude,
  ...
  private canStartTour(ps: PositionSimulator) {
    ...
    if (this.selectedTour.keyPoints.length != 0) {
      ...
      if (
        this.selectedTour.id != undefined &&
        this.mapComponent.getDistance(
          firstKeyPointLat!,
          firstKeyPointLng!,
          ps.latitude,
    ...
    if (this.activeSession != null) {
      this.selectedTour.keyPoints.forEach((kp) => {
        if (
          ...
          this.activeSession.completedKeyPoints.find(
            (ckp) => ckp.keyPointId == kp.id
          ) == undefined &&
          kp.positionInTour === this.activeSession.completedKeyPoints.length
        ) {
          console.log('ID OD KP: ', kp.id);
          this.service
            .completeKeyPoint(this.activeSession.id!, kp.id!)
            .subscribe({
              next: (result: Session) => {
                this.activeSession = result;
                this.canCompleteTour();
              ...
              complete: () => {
                setTimeout(() => {
                  this.toastr.success(
                    'Click on keypoint marker to reveal a secret.',
                    'Keypoint completed!'
                  );
                }, 800);
              },
  ...
  private canCompleteTour() {
    if (
      this.activeSession != null &&
      this.activeSession.completedKeyPoints.length ==
        this.selectedTour.keyPoints.length
    )
      this.showCompleteTour = true;
    else this.showCompleteTour = false;
  }
  ...
  private setTourDistanceCrossed() {
    if (this.activeSession != null) {
      ...
      for (let keyPoint of this.selectedTour.keyPoints) {
        const isCompleted = this.activeSession.completedKeyPoints.some(
          (completedPoint) => completedPoint.keyPointId === keyPoint.id
        );
      ...
      setTimeout(() => {
        this.activeSession.distanceCrossedPercent = Math.round(
          (this.routeMap.getRouteDistanceInMeters() /
            (this.selectedTour.distanceInKm * 1000)) *
            100
        );
        this.service.updateSession(this.activeSession).subscribe({
          next: (result: Session) => {
  ...
  showNotificationForBlog() {
    const dialogRef = this.snackBar.open(
      'Would you like to write a blog about this tour?',
      'Yes',
      {
        duration: 15000,
  ...
  openReviewForm(tour: Tour) {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result === 'success') {
        ...
        this.toastr.error(
          'There was an error while trying to add a rating',
          'Error'
        );
  ...
  showChallengeOnMap() {
    this.mapComponent.removeFacilities();
    this.displayedChallenges = this.challenges.filter(
      (challenge) => challenge.type == parseInt(this.selectedChallengeType)
    );
  ...
  startChallenge() {
    ...
    this.service.addChallengeExecution(challengeExecution).subscribe({
      ...
      error: () => {
        this.toastr.error(
          'There was an error while trying to activate a challenge',
          'Error'
        );
      },
      complete: () => {
        ...
        if (this.nearbyChallenge?.type === ChallengeType.Social) {
          if (this.activeChallenge?.isCompleted) {
            this.toastr.success(
              `You have completed social challenge: ${
                this.nearbyChallenge!.name
              }`,
              'Success'
            );
  ...
  openChallengeInfo() {
    if (this.nearbyChallenge?.type === ChallengeType.Social) {
      this.openSocialChallengeDialog(
        this.nearbyChallenge!.name!,
        this.nearbyChallenge!.description!,
        this.nearbyChallenge.requiredAttendance!
      );
      this.toastr.info(
        `Wait for ${this.nearbyChallenge.requiredAttendance} people to arrive`,
        'Info'
      );
    }
    ...
    if (this.nearbyChallenge?.type === ChallengeType.Location) {
      this.openLocationChallengeDialog(
        this.nearbyChallenge!.name!,
        this.nearbyChallenge!.description!,
        this.nearbyChallenge!.image!
      );
    }
    if (this.nearbyChallenge?.type === ChallengeType.Misc) {
      this.openMiscChallengeDialog(
        this.nearbyChallenge!.name!,
        this.nearbyChallenge!.description!
      );
    }

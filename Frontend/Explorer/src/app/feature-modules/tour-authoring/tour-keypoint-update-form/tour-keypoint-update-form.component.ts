...
export class TourKeypointUpdateFormComponent {
  ...
  ngOnInit() {
    ...
    if (this.tourKeypointToUpdate.id != null) {
      this.service
        .getTourKeyPointById(this.tourKeypointToUpdate.id)
        .subscribe((result: TourKeyPoints) => {
          ...
          this.newKeyPoint = this.mapComponent.initKeyPointMarker(
            result.latitude,
            result.longitude
          );
        });
  ...
  updateKeyPoint() {
    if (
      this.newKeyPoint.latitude != this.tourKeypointToUpdate.latitude &&
    ...
    if (
      this.newKeyPoint.longitude != this.tourKeypointToUpdate.latitude &&
    ...
    if (this.tourKeyPointForm.valid) {
      ...
      this.tourKeypointToUpdate.description =
        this.tourKeyPointForm.value.description || '';
      this.tourKeypointToUpdate.image = this.tourKeyPointForm.value.image || '';

      console.log('KP ZA UPDATE: ', this.tourKeypointToUpdate);
      this.service.updateTourKeyPoint(this.tourKeypointToUpdate).subscribe({
        ...
        error: () => {
          this.toastr.error(
            'There was an error while trying to update a keypoint',
            'Error'
          );
        },
      });
